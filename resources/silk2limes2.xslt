...
<xsl:stylesheet version = '2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'>
		...
		<LIMES><xsl:value-of select="$newline"/>
			...
			<SOURCE>
				<xsl:for-each select="DataSources/DataSource[@id=$sourceID][1]">
					<xsl:call-template name="SourceDatasource"/>
				</xsl:for-each>
			...
			<TARGET>
				<xsl:for-each select="DataSources/DataSource[@id=$targetID][1]">
					<xsl:call-template name="TargetDatasource"/>
				</xsl:for-each>
				<xsl:call-template name="getProperties">
			...
			<OUTPUT>
				<xsl:for-each select="Interlinks/Interlink/Outputs/Output[1]/Param[@name='format']">
					<xsl:choose>
						<xsl:when test="@value='ntriples'">NT</xsl:when>
	...
	<xsl:template name="SourceDatasource">
		<!--            <xsl:param name="id"/> -->
		...
		<GRAPH>
			<xsl:choose>
				<xsl:when test="Param[@name='graph']">
	...
	<xsl:template name="TargetDatasource">
		<!--            <xsl:param name="id"/> -->
		<!-- id and endpoint must be always there both in Silk and in LIMES-->
		<ID>
			<xsl:value-of select="@id"/>
	...
	<xsl:template name="LinkCondition" match="LinkCondition">
		<xsl:choose>
			<xsl:when test="@metric='JaroWinkler'">trigrams</xsl:when>
			<xsl:when test="@metric='Jaro'">trigrams</xsl:when>
			<xsl:when test="@metric='Levenshtein'">levenshtein</xsl:when>
			<xsl:otherwise>trigrams(</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
